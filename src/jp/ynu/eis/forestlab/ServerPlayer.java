...
public class ServerPlayer {
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, ClassNotFoundException, IOException {
		...
		ServerPlayer ssbi = new ServerPlayer();
		ssbi.start();
		ssbi.result();
		System.exit(1);
	}
	...
	public ServerPlayer() throws IOException{
		...
		File initFile = new File("res\\AutoStarter.ini");
...
//		}*/
	...
	public void start(int i) throws SocketTimeoutException, IOException, InstantiationException, IllegalAccessException, ClassNotFoundException{
		//startServer();
		startClient("Forest0");
		startClient("Forest1");
		startClient("Forest2");
	...
	private void startClient() throws InstantiationException, IllegalAccessException, ClassNotFoundException {
		...
		Role role = null;
		MyRoleAssignPlayer myPlayer = new MyRoleAssignPlayer();
		MyBasePlayer myBase = new MyBasePlayer();
		String playerName = myPlayer.getName();
		
		//ローカルホスト用
		//TcpipClient client = new TcpipClient("localhost", port);
		
		//ネットワーク通信用
		TcpipClient client = new TcpipClient("160.16.83.206", 10001);
		//TcpipClient client = new TcpipClient("192.168.1.23", 10001);
		
		if(playerName != null){
			client.setName(playerName);
//			System.out.println("Set name "+client.getName());
		}
		if(client.connect(myPlayer)){
...
//				System.out.println("Player connected to server:"+player);
	...
	private void startClient(String name) throws InstantiationException, IllegalAccessException, ClassNotFoundException {
